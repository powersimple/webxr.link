...
'resources'
	...
	function cleanHostName($filepath){
		$filepath = str_replace("http://","",$filepath);
						$filepath = str_replace("https://","",$filepath);
						$filepath = str_replace($_SERVER['HTTP_HOST'],"",$filepath);
						return $filepath;
	}
		...
		function getThumbnailVersions($id){
				...
				foreach(get_intermediate_image_sizes() as $key => $size){//loop through sizes
					...
					if($img[0] !=""){
						...
						$thumbnail_versions[$size]=cleanHostName($img[0]);//cleaned version of url sans host path
					} 
				}
